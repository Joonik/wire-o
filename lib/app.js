...
*/
const mergeUrls = async (applicants,resumeUrls,opportunity, filePaths) => {
  //Implementation for User Summaries and summary table.
  const summariesPaths  = await createSummaries(applicants, opportunity);
  ...
  if(key)await uploadPDF(buffer, (opportunity?.slug));
  return buffer;
};
...
exports.handler = (event, context, callback) => {
  ...
  const merge = mergeUrls(event.body.applicants,event.body.pdfUrls,event.body.opportunity, []);
